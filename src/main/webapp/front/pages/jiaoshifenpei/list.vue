<template>
    ...
    <view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
			<picker v-if="queryList.length>1" mode="selector" :range="queryList" range-key="queryName" :value="queryIndex" @change="queryChange" style="padding-left: 20upx;">
				...
				<input v-model="searchForm.jiaoxueloumingcheng" type="text" placeholder="教学楼名称" ></input>
			</view>
                  			<view v-if="queryIndex==1" class="search-form round">
				<text class="cuIcon-search"></text>
				<input v-model="searchForm.jiaoshihao" type="text" placeholder="教室号" ></input>
			</view>
        ...
        <view @tap="onDetailTap(product)" class="uni-product" :style='{"padding":"0px 20rpx 0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0 1% 20rpx 2%","borderColor":"rgba(130, 163, 157, 1)","backgroundColor":"#fff","borderRadius":"20rpx","borderWidth":"2rpx","width":"47%","borderStyle":"solid","height":"auto"}' v-for="(product,index) in list" :key="index">
                                <view class="uni-product-title" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","color":"#333","textAlign":"center","borderRadius":"0","borderWidth":"0","width":"100%","fontSize":"28rpx","lineHeight":"56rpx","borderStyle":"solid"}'>{{product.jiaoxueloumingcheng}}</view>
                                          <view class="uni-product-title" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","color":"#333","textAlign":"center","borderRadius":"0","borderWidth":"0","width":"100%","fontSize":"28rpx","lineHeight":"56rpx","borderStyle":"solid"}'>{{product.jiaoshihao}}</view>
                                                                                                                                                      <view style="display: flex;justify-content: space-between;">
            <text v-if="userid && isAuth('jiaoshifenpei','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="!userid && isAuthFront('jiaoshifenpei','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="userid && isAuth('jiaoshifenpei','删除')" class="cuIcon-delete" @click.stop="onDeleteTap(product.id)">删除</text>
            <text v-if="!userid && isAuthFront('jiaoshifenpei','删除')" class="cuIcon-delete" @click.stop="onDeleteTap(product.id)">删除</text>
          </view>
        </view>
      </view>
            <!-- 样式2 -->
	...
	export default {
		...
		methods: {
			...
			queryChange(e) {
				...
				this.searchForm.jiaoxueloumingcheng="";
				this.searchForm.jiaoshihao="";
			...
			async upCallback(mescroll) {
				...
				if(this.searchForm.jiaoxueloumingcheng){
					params['jiaoxueloumingcheng'] = '%' + this.searchForm.jiaoxueloumingcheng + '%'
				}
				if(this.searchForm.jiaoshihao){
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`jiaoshifenpei`, params);
                } else {
                    res = await this.$api.list(`jiaoshifenpei`, params);
                }
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('jiaoshifenpei', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
				...
				if(this.searchForm.jiaoxueloumingcheng){
					searchForm['jiaoxueloumingcheng'] = '%' + this.searchForm.jiaoxueloumingcheng + '%'
				}
				if(this.searchForm.jiaoshihao){
					searchForm['jiaoshihao'] = '%' + this.searchForm.jiaoshihao + '%'
				}
				if(this.searchForm.xueshengxingming){
                ...
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`jiaoshifenpei`, searchForm);
                } else {
                    res = await this.$api.list(`jiaoshifenpei`, searchForm);
                }
				// 如果是第一页数据置空
