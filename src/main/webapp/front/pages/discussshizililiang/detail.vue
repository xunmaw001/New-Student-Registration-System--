...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussshizililiangCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussshizililiangCleanType')
				this.mescroll.num = 1
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discussshizililiang');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','discussshizililiang');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('discussshizililiang', this.id);
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscussshizililiang', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussdiscussshizililiang/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('discussshizililiang', this.detail);
				this.$utils.msg('审核成功');
