<template>
  <div class="main-content">
        ...
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
            ...
            <el-table-column width="300" :align="contents.tableAlign" 
               :header-align="contents.tableAlign"
                ...
                <el-button v-if="isAuth('xuesheng','缴费') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="zaixianjiaofeiCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'缴费':'' }}</el-button>
                <el-button v-if="isAuth('xuesheng','缴费') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="zaixianjiaofeiCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'缴费':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('xuesheng','缴费') && contents.tableBtnIcon == 0" type="success" size="mini" @click="zaixianjiaofeiCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'缴费':'' }}</el-button>
    ...
    <zaixianjiaofei-cross-add-or-update v-if="zaixianjiaofeiCrossAddOrUpdateFlag" :parent="this" ref="zaixianjiaofeiCrossaddOrUpdate"></zaixianjiaofei-cross-add-or-update>
...
import zaixianjiaofeiCrossAddOrUpdate from "../zaixianjiaofei/add-or-update";
export default {
  data() {
    return {
      searchForm: {
      ...
      zaixianjiaofeiCrossAddOrUpdateFlag: false,
  ...
  components: {
    AddOrUpdate,
    zaixianjiaofeiCrossAddOrUpdate,
  },
  methods: {
    ...
    zaixianjiaofeiCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.zaixianjiaofeiCrossAddOrUpdateFlag = true;
		...
		for (var o in obj){
		  if(o==statusColumnName && obj[o]==statusColumnValue){
			...
			this.getDataList();
		      ...
		      this.showFlag = true;
		      this.zaixianjiaofeiCrossAddOrUpdateFlag = false;
			return;
		  }
		}
	}
      this.$nextTick(() => {
      this.$refs.zaixianjiaofeiCrossaddOrUpdate.init(row.id,type);
      });
    },
    init () {
